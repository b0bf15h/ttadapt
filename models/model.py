...
import sys
sys.path.append('/home/bob/test-time-adaptation/classification/mixmo_pytorch')
#from mixmo.networks.wrn import WideResNett

config_network = {'depth': 28, 'widen_factor':10}
config_args = {'num_members':1, 'data':{'dataset': 'cifar100', 'num_classes':100}}

# import mixmo_pytorch.mixmo.networks.wrn
# import mixmo_pytorch.mixmo.networks.resnet

#from mixmo_pytorch.mixmo.networks import resnet, wrn 
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    elif cfg.MODEL.ARCH == 'WideResNet':
                        base_model = WideResNet()
                        checkpoint_dir = cfg.MODEL.CKPT_PATH
                    ...
                    elif cfg.MODEL.ARCH == 'Standard_CIFAR100':
                        base_model = timm.create_model("resnet34_cifar100", pretrained=True)
                        #base_model = WideResNett(config_args = config_args, config_network = config_network)
                        # base_model = WideResNet(num_classes=100)
                        # base_model.load_state_dict(torch.load('/home/bob/test-time-adaptation/classification/ckpt/cifar100/checkpoint_cifar100_wrn2810_1net_standard_bar1.ckpt'), strict=False)
                    else:
                    ...
                    print('-'*60)
                    print('using robustbench')
                    dataset_name = cfg.CORRUPTION.DATASET.split("_")[0]
                    base_model = load_model(cfg.MODEL.ARCH, cfg.CKPT_DIR, dataset_name, ThreatModel.Linf)
...
def split_up_model(model, arch_name: str, dataset_name: str):
    ...
    if hasattr(model, "model") and hasattr(model.model, "pretrained_cfg") and hasattr(model.model, model.model.pretrained_cfg["classifier"]):
        ...
        classifier = model.fc
    elif "resnet" in arch_name or "resnext" in arch_name or "wide_resnet" in arch_name or arch_name in {"Standard_R50", "Hendrycks2020AugMix", "Hendrycks2020Many", "Geirhos2018_SIN", "Cui2023Decoupled_WRN-34-10_autoaug"}:
