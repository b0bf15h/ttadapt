...
def make_custom_dataset(root, path_imgs, cls_dict):
    #path_imgs = os.path.join(os.getcwd(),'classification', path_imgs)
    #cls_dict = os.path.join(os.getcwd(),'classification', cls_dict)
    path_imgs = os.path.join(os.getcwd(), path_imgs)
    cls_dict = os.path.join(os.getcwd(), cls_dict)
...
def pil_loader(path):
    ...
    # BELOW LINES BEFORE WITH OPEN ARE MY OWN CODE
    parts = path.split('/')

    # Filter out the part that starts with 'n'
    filtered_parts = [part for part in parts if not part.startswith('n')]
