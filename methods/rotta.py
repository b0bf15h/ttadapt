...
class RoTTA(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        # self.model_src = self.copy_model(self.model)
    ...
    def forward_and_adapt(self, x):
        ...
        for i, data in enumerate(imgs_test):
            ...
            if self.current_instance % self.update_frequency == 0:
                self.update_model()
        # prior = ema_out.softmax(1).mean(0)
        # smooth = max(1 / ema_out.shape[0], 1 / ema_out.shape[1]) / torch.max(prior)
        # smoothed_prior = (prior + smooth) / (1 + smooth * ema_out.shape[1])
        # return ema_out * smoothed_prior
        return ema_out
    ...
    def update_model(self):
        ...
        if len(sup_data) > 0:
            ...
            l_sup = (softmax_cross_entropy(stu_sup_out, ema_sup_out, 0.5) * instance_weight).mean()
    ...
    def update_ema_variables(ema_model, model, nu):
        ...
        # for param, src_param in zip(ema_model.parameters(), src_model.parameters()):
        #     if param.requires_grad:
        #         param.data[:] = 0.975 * param[:].data[:] + 0.025 * src_param[:].data[:].to(device)
        return ema_model
...
@torch.jit.script
def symmetric_cross_entropy(x,x_ema, alpha:float):
